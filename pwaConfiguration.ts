...
const pwaConfigurationFactory = (
  ...
  const notValidManifest = (...args: string[]): boolean => {
    return !args.every(arg => Reflect.has(newPwaConfiguration.manifest as {}, arg))
  }

  if (notValidManifest('name', 'short_name', 'description')) return
  const manifest = newPwaConfiguration.manifest as any
  manifest['name'] = siteName 
  manifest['short_name'] = siteShortName
  manifest['description'] = siteDescription

  if(!newPwaConfiguration.workbox) return

  if (build) {
    newPwaConfiguration.workbox.globDirectory = '.output/public/'
    newPwaConfiguration.outDir = '.output/public/_nuxt/'
    pages && (newPwaConfiguration.workbox.additionalManifestEntries = pages.reduce((acc: any[], me) => {
      acc.push(me)
      return acc
    }, []))
  }
